...
export default function ListeningPage() {
  ...
  useEffect(() => { return () => { killEverything() } }, [])

  const killEverything = useCallback(() => {
    isActiveRef.current = false
    if (typingIntervalRef.current) { clearInterval(typingIntervalRef.current); typingIntervalRef.current = null }
    if (currentSourceRef.current) { try { currentSourceRef.current.onended = null; currentSourceRef.current.stop() } catch {} currentSourceRef.current = null }
    if (audioCtxRef.current && audioCtxRef.current.state !== 'closed') { try { audioCtxRef.current.suspend() } catch {} }
    if (mediaRecorderRef.current) { try { mediaRecorderRef.current.onstop = null; mediaRecorderRef.current.ondataavailable = null; if (mediaRecorderRef.current.state !== 'inactive') mediaRecorderRef.current.stop() } catch {} mediaRecorderRef.current = null }
  ...
  const goBack = useCallback((targetPhase: 'menu' | 'lang' | 'pick' | 'pronunciation') => {
    ...
    if (targetPhase === 'pronunciation') { router.push('/pronunciation') } else { setPhase(targetPhase) }
  }, [killEverything, router])
  ...
  const playTTS = async (text: string, voice = 'kalina', dictationId?: string, langOverride?: string): Promise<void> => {
    if (!isActiveRef.current) return
    const res = await fetch('/api/tts-azure', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, voice, speed: 0.85, dictation_id: dictationId, lang: langOverride }) })
    if (!isActiveRef.current) return
    ...
    return new Promise<void>((resolve) => {
      ...
      ctx.decodeAudioData(arrayBuffer, (decoded) => {
        ...
        gainNode.gain.value = peak > 0 ? Math.min(1.0 / peak * 2.0, 5.0) : 2.0
  ...
  const typeText = (text: string): Promise<void> => {
    return new Promise<void>((resolve) => {
      ...
      typingIntervalRef.current = setInterval(() => {
        if (!isActiveRef.current) { if (typingIntervalRef.current) clearInterval(typingIntervalRef.current); resolve(); return }
        i++; setTypedText(text.slice(0, i))
        if (i >= text.length) { if (typingIntervalRef.current) clearInterval(typingIntervalRef.current); resolve() }
      }, 38)
    })
  }
  ...
  const acquireMic = async (): Promise<MediaStream | null> => {
    try {
      if (currentStreamRef.current && currentStreamRef.current.getTracks().some(t => t.readyState === 'live')) return currentStreamRef.current
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true })
      currentStreamRef.current = stream; return stream
    } catch { return null }
  }
  ...
  const startRecording = async (onStop: (blob: Blob, mimeType: string) => void) => {
    ...
    const mr = new MediaRecorder(stream, { mimeType }); mediaRecorderRef.current = mr
    mr.ondataavailable = (e) => { if (e.data.size > 0) chunksRef.current.push(e.data) }
    mr.onstop = () => { const blob = new Blob(chunksRef.current, { type: mimeType }); onStop(blob, mimeType) }
    mr.start(); setRecording(true); setWaitingForSpeech(false)
  }
  ...
  const startAlphabet = async (selectedLang: Lang) => {
    await unlockAudio(); isActiveRef.current = true
    setMode('alphabet'); setAlphaIndex(0); setScore(0); setFeedbackType(''); setOwlSays(''); setPhase('play')
  ...
  const handleAlphaTap = async (idx: number, selectedLang: Lang) => {
    const alpha = getAlphabet(selectedLang)
    await startRecording(async (blob, mimeType) => {
      if (!isActiveRef.current) return
      const fd = new FormData(); const ext = mimeType.includes('mp4') ? 'mp4' : 'webm'
      fd.append('file', blob, `audio.${ext}`); fd.append('language', LANG_CONFIG[selectedLang].whisperLang)
    ...
    setTimeout(() => { if (isActiveRef.current) stopRecording() }, 4000)
  }

  const skipAlpha = async (idx: number, selectedLang: Lang) => {
    if (!isActiveRef.current) return
    killEverything(); isActiveRef.current = true
    const alpha = getAlphabet(selectedLang); const next = idx + 1
    if (next >= alpha.length) { setPhase('done'); return }
    setAlphaIndex(next); setFeedbackType(''); setOwlSays('')
    await playTTS(alpha[next].tts_text || alpha[next].word, 'kalina', undefined, LANG_CONFIG[selectedLang].dictLang)
    if (!isActiveRef.current) return; setWaitingForSpeech(true)
  ...
  const handleSentenceTap = async (dictation: Dictation, idx: number) => {
    await startRecording(async (blob, mimeType) => {
      if (!isActiveRef.current) return
      const fd = new FormData(); const ext = mimeType.includes('mp4') ? 'mp4' : 'webm'
      fd.append('file', blob, `audio.${ext}`); fd.append('language', lang ? LANG_CONFIG[lang].whisperLang : 'bg')
      const res = await fetch('/api/whisper', { method: 'POST', body: fd })
      if (!isActiveRef.current) return
      const data = await res.json()
      const transcript = (data.text || '').toLowerCase().trim()
  ...
  const skipSentence = async () => {
    ...
    setSentenceIndex(next); await playSentence(selected, next)
  }
  ...
  const loadLevel = async (lvl: 'easy' | 'medium' | 'hard', selectedLang: Lang) => {
    ...
    setDictations(filtered); setMode(lvl); setPhase('pick')
  }
      ...
      <main className="u4a-dash min-h-screen flex flex-col items-center justify-center p-6">
        ...
        <div className="w-full max-w-md text-center" style={{ position: 'relative', zIndex: 1 }}>
          ...
          {(['easy', 'medium', 'hard'] as const).map(lvl => {
            ...
            return (
              <button key={lvl} onClick={async () => { await acquireMic(); await unlockAudio(); loadLevel(lvl, lang) }}
      ...
      <div className="max-w-md mx-auto" style={{ position: 'relative', zIndex: 1 }}>
        ...
        {dictations.map(d => (
          <button key={d.id} onClick={async () => {
            await acquireMic(); await unlockAudio(); isActiveRef.current = true
            setSelected(d); setSentenceIndex(0); setScore(0); setFeedbackType(''); setOwlSays(''); setTypedText('')
            setPhase('play'); playSentence(d, 0)
  ...
  if (phase === 'play' && selected && cfg) {
    ...
    return (
      <main className="u4a-dash min-h-screen flex flex-col items-center p-6 pt-10">
        ...
        <div className="w-full max-w-md" style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <button onClick={() => goBack('pick')} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer', color: cfg.color }}>←</button>
          ...
          {feedbackType === 'wrong' && !recording && !loading && !waitingForSpeech && (
            <button onClick={skipSentence}
              style={{ width: '100%', background: 'none', border: `2px solid ${cfg.colorBorder}`, color: cfg.color, borderRadius: 16, padding: '0.9rem', fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '1rem', cursor: 'pointer' }}>
              Пропусни →
            </button>
          )}
        </div>
    ...
    return (
      <main className="u4a-dash min-h-screen flex flex-col items-center justify-center p-6">
        ...
        <div className="w-full max-w-md text-center" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <button onClick={() => { killEverything(); isActiveRef.current = true; setPhase(mode === 'alphabet' ? 'lang' : 'pick'); setSelected(null) }}
            style={{ width: '100%', background: c.gradient, color: 'white', border: 'none', borderRadius: 20, padding: '1.2rem', fontFamily: 'Nunito, sans-serif', fontWeight: 900, fontSize: '1.3rem', cursor: 'pointer', boxShadow: `0 8px 24px ${c.shadow}`, marginBottom: 12 }}>
            Избери друг текст
          </button>
          <button onClick={() => router.push('/pronunciation')}
            style={{ width: '100%', background: 'none', border: '2px solid #FDE68A', color: '#F97316', borderRadius: 20, padding: '1rem', fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '1.1rem', cursor: 'pointer' }}>
            ← Към Правоговор
          </button>
        </div>
      </main>
